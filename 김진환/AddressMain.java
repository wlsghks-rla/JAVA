	...
	public static void main(String[] args) {
		...
		while(run) {
			...
			case 3:								
					...
					for(int i=0; i<friends.length; i++) {
						if((friends[i].getNumber()).equals(number)) {
							System.out.printf("%-4s\t%-20s\t%s\n", friends[i].getName(), friends[i].getHome(), friends[i].getNumber());
							break;
						}
					}
			...
			case 4:						
					...
					for(int i=0; i<friends.length; i++) {
						if((friends[i].getName()).equals(searchName)) {
							count++;
							String infoName = friends[i].getName();
							String infoAddress = friends[i].getHome();
							String infoNumber = friends[i].getNumber();
							System.out.println("----------");
